...
//void *v   v pointer to the return value of my_seq_start, that is &counter
static void *my_seq_next(struct seq_file *s, void *v, loff_t * pos)
{
    unsigned long *tmp_v = (unsigned long *) v;
    printk("Invoke seq next pos = %d\n", *pos);
    if (*pos < MAX_LINES) {
	(*tmp_v)++;
	(*pos)++;
	return tmp_v;
    } else {
    printk("Invoke seq next end\n");
	*pos = 0;
	(*tmp_v) = 0;
	return NULL;
    }
...
int init_module(void)
{
    ...
    printk(KERN_INFO "Initialze /proc/test_proc success!\n");
    return 0;
}
...
void cleanup_module(void)
{
    ...
    printk(KERN_INFO "Remove /proc/test_proc success!\n");
}
